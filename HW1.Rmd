...
if (!is.na(CI)) {
  if (CI == 95) {
    p <- p_temp + 
      geom_ribbon(data = est_temp, aes(x = Year, ymin = L95, ymax = U95, alpha = 0.3))
  } else if (CI == 80) {
    p <- p_temp + 
      geom_ribbon(data = est_temp, aes(x = Year, ymin = L80, ymax = U80, alpha = 0.3))
  }
...
plot_cp(dat2, est, iso_code = 4)
plot_cp(dat2, est, iso_code = 4, CI = NA)
plot_cp(dat2, est, iso_code = 404, CI = 80)
```
