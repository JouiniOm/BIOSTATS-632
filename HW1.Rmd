...
setwd("~/Studium/Public Health Studium/5. Semester UMass/R III Course")
```
...
est$iso_country <- as.integer(est$iso_country)
...
plot_cp <- function(est, dat, iso_code, CI = 95) {
  ...
  p_temp <- ggplot() +
    geom_line(data = est_temp, aes(x = Year, y = Median), color = "blue") +
    geom_point(data = dat_temp, aes(x = ref_time, y = contraceptive_use_modern), color = "red") +
    labs(x = "Time", y = "Modern use (%)", title = paste("mCPR in", est_temp$`Country or area`[1]))
  
    # Conditional CI layers
if (!is.na(CI)) {
  if (CI == 95) {
    p <- p_temp + geom_ribbon(data = est_temp, aes(x = Year, ymin = L95, ymax = U95))
  } else if (CI == 80) {
    p <- p_temp + geom_ribbon(data = est_temp, aes(x = Year, ymin = L80, ymax = U80))
  }
  # If CI is neither 95 nor 80, or if CI == NA, no CI layer is added. 
  # Additional modifications like fill color and transparency (alpha) can be adjusted as needed.
}
  return(p)
}
```
...
[Github-Link]https://github.com/JouiniOm/BIOSTATS-632/blob/main/HW1.Rmd
